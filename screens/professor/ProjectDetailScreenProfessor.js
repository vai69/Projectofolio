...
export default function ProjectDetailScreenProfessor(props) {
  ...
  const tech = project.TechStack.map((item, key) => {
    return (<TechItem tech={item} />)
  })
      ...
      <View style={styles.container}>
        ...
        <View style={styles.contributorContainer}>
          <View style={styles.entity}>
            ...
            <View style={{ marginLeft: 5 }}>
              {
                project.Contributor.map((item) => {
                  return (
                    <TouchableNativeFeedback onPress={() => props.navigation.navigate('ProfileCheckByProfessor',{contributor: item})}>
                      <Text style={styles.entityContent}>
                        {item}
                      </Text>
                    </TouchableNativeFeedback>
