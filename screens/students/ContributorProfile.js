...
export default function ContributorProfile(props) {
  ...
  const userRef = () => firebase.firestore().collection('students').get().then(snapshot => snapshot.docs.map(doc => {
    if (doc.data().Email === props.route.params.contributor) {
      setUser(doc.data())
      return doc.data();
    }
  }));

  const loadContributo = () => {
    userRef();
  }
  useEffect(() => {
    loadContributo();
  ...
  const Queuse = projects.filter(project => project.Contributor.includes(props.route.params.contributor));
